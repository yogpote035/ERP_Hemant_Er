 ...
 */
import type { Id, Invoice } from '../types/domain.js'
...
export function selectBillingTotals(s: RootState, allowed: Set<Id> | null = null): BillingTotals {
  let invoiced = ZERO
  ...
  for (const inv of values(s.billing.invoices)) {
    if (allowed && !allowed.has(inv.unitId)) continue
