 ...
 * default used by the outward form. All DERIVED.
 *
 * Unit scope is passed explicitly as `allowed?: Set<Id> | null` (null = no
 * scoping; the route layer supplies `req.auth.allowedUnitIds`).
 */
...
function scopedInwards(s: RootState, allowed: Set<Id> | null): Inward[] {
...
function scopedDispatches(s: RootState, allowed: Set<Id> | null): Dispatch[] {
...
export function selectInwardRows(s: RootState, allowed: Set<Id> | null = null): InwardRow[] {
...
export function selectOpenInwardRows(s: RootState, allowed: Set<Id> | null = null): InwardRow[] {
...
export function selectStockRows(
  ...
  from?: string,
  to?: string,
  allowed: Set<Id> | null = null
): StockRow[] {
  const seen = new Set<string>()
  const rows: StockRow[] = []
  const add = (unitId: Id, partId: Id) => {
    if (allowed && !allowed.has(unitId)) return
...
export function selectDispatchRows(s: RootState, allowed: Set<Id> | null = null): DispatchRow[] {
  const invoices = values(s.billing.invoices)
