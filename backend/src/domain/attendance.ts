 ...
 */
import type { Id, ISODate, ProductionAttendance, ShiftAttendance } from '../types/domain.js'
...
export function selectProductionRows(s: RootState, allowed: Set<Id> | null = null): ProductionRow[] {
  return values(s.hr.production)
    .filter((a) => !allowed || allowed.has(a.unitId))
    .map((a) => ({
      entry: a,
...
export function selectShiftRows(s: RootState, allowed: Set<Id> | null = null): ShiftRow[] {
 ...
 */
export function selectEarningsBetween(
  s: RootState,
  from?: ISODate,
  to?: ISODate,
  ...
  for (const row of selectProductionRows(s, allowed)) {
  ...
  for (const row of selectShiftRows(s, allowed)) {
    if (!inRange(row.entry.date)) continue
    const r = get(row.entry.employeeId)
...
export function selectEarnings(s: RootState, allowed: Set<Id> | null = null): EarningRow[] {
  return selectEarningsBetween(s, undefined, undefined, allowed)
}
