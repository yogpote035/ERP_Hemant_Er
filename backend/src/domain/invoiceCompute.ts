 ...
 * round-off + grand, and the packing-box breakdown.
 *
 * Unit scope (for `selectInvoiceRows`) is passed explicitly as
 * `allowed?: Set<Id> | null` (null = no scoping).
 */
import type { Id, Invoice, InvoiceTotals, PackingBox, TaxKind } from '../types/domain.js'
import {
  addP,
  mulQty,
  pctOfPaise,
...
import { formatDMY, toWordsIndian } from './format.js'
import { values, getById } from '../db/normalized.js'
import type { RootState } from '../db/state.js'
import { invoicePaymentStatus, outstandingForInvoice, invoiceGrand, type PaymentStatus } from './billing.js'
...
export function selectInvoiceDocModel(s: RootState, invoiceId: Id): InvoiceDocModel | null {
  ...
  return {
    ...
    motorVehicleNo: undefined,
    dispatchedThrough: undefined,
    destination: custAddress.length ? custAddress[custAddress.length - 1] : undefined,
...
/** Scoped invoice list for the Billing page (null = no scoping). */
export function selectInvoiceRows(s: RootState, allowed: Set<Id> | null = null): InvoiceRow[] {
