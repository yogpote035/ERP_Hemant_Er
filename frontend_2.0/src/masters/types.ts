...
export interface MasterConfig<T extends BaseEntity, F extends FieldValues> {
  ...
  fields: FieldSpec[]
  /** When set, the list shows a search box that matches against this text (case-insensitive). */
...
export interface MasterView {
  ...
  fields: FieldSpec[]
  schema: ZodType
  /** When set, the list shows a search box matching this text (case-insensitive). */
  searchText?: (row: BaseEntity) => string
  searchPlaceholder?: string
