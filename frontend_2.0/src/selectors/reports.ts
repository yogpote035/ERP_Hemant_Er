 ...
 * builder over the (already unit-scoped) derived selectors from P2-P6, returning
 * display columns + plain rows ready for both the on-screen table and the Excel
 * export. Date-bearing reports honour the date range; aggregates ignore it.
 */
import type { RootState } from '@/store/state'
import { getById } from '@/store/normalized'
import { addP, formatINRSymbol, type Paise } from '@/lib/money'
import { formatDMY } from '@/lib/date'
import { selectInwardRows, selectStockRows } from './register'
import { selectInvoiceRows } from './invoiceCompute'
import { selectScrapRows, selectExpenseRows, scrapMathOf } from './finance'
import { selectEarningsBetween } from './attendance'
...
export interface ReportDef {
  ...
  build: (s: RootState, range: DateRange) => ReportResult
}
...
export const REPORTS: ReportDef[] = [
  ...
  },
  {
    ...
    build: (s, r) => {
      ...
      for (const row of selectInvoiceRows(s)) {
  ...
  },
  {
    ...
    build: (s) => ({
      ...
      rows: selectStockRows(s).map((row) => ({
  ...
  {
    // SRS FR-AT20 — period-wise labour payment summary, employee-wise, consolidating
    // production-based earnings and shift/time-based wages for the selected date range.
    key: 'attendance-earnings',
    label: 'Labour payment summary',
    dateFiltered: true,
    build: (s, r) => ({
      columns: [
        { key: 'employee', label: 'Employee' },
        { key: 'production', label: 'Production earnings', align: 'right' },
        { key: 'shift', label: 'Shift / time wages', align: 'right' },
        { key: 'total', label: 'Total payable', align: 'right' },
      ],
      rows: selectEarningsBetween(s, r.from || undefined, r.to || undefined).map((row) => ({
