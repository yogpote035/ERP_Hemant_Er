 ...
 */
function reconcileCore(s: RootState, allowed: Set<Id> | null): ReconcileResult {
  const keys = allowed
...
export function selectReconcile(s: RootState): ReconcileResult {
  return reconcileCore(s, null)
}

/**
 * Reconciliation limited to the units the current session may read (plan §2.6).
 * This is what the dashboard + sidebar badge use, so a non-admin never sees
 * imbalance counts from units outside their scope. An admin focused on 'ALL'
 * gets the full cross-unit view (incl. orphans).
 */
export function selectReconcileScoped(s: RootState): ReconcileResult {
  const user = currentUser(s)
  if (!user) return { ok: true, redCount: 0, ledgers: [], orphanDispatchIds: [] }
  const adminAll =
    user.role === 'admin' && (!s.session.currentUnitId || s.session.currentUnitId === 'ALL')
  return reconcileCore(s, adminAll ? null : allowedUnitIds(s))
}
