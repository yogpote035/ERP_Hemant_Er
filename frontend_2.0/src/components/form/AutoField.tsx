...
export function AutoField({ field, disabled = false, hint }: { field: FieldSpec; disabled?: boolean; hint?: string }) {
  const renderedField = hint === undefined ? field : { ...field, hint }
  switch (field.kind) {
    case 'text':
    case 'textarea':
      return <TextLike field={renderedField as Extract<FieldSpec, { kind: 'text' | 'textarea' }>} />
    case 'number':
    case 'money':
      return <NumberLike field={renderedField as Extract<FieldSpec, { kind: 'number' | 'money' }>} />
    case 'date':
      return <DateField field={renderedField as Extract<FieldSpec, { kind: 'date' }>} />
    case 'select':
      return <SelectField field={renderedField as Extract<FieldSpec, { kind: 'select' }>} disabled={disabled} />
    case 'checkbox':
  ...
  return (
    ...
    >
      <Input
        ...
        aria-describedby={desc}
        {...register(field.name, {
          // Blank values remain undefined; supplied values are converted to numbers.
          setValueAs: (v) => {
...
function DateField({ field }: { field: Extract<FieldSpec, { kind: 'date' }> }) {
  ...
  return (
    <Field
      ...
      className={colSpan(field.colSpan)}
    >
      <Input id={field.name} type="date" aria-invalid={msg ? true : undefined} aria-describedby={desc} {...register(field.name)} />
    </Field>
  )
}

function SelectField({ field, disabled = false }: { field: Extract<FieldSpec, { kind: 'select' }>; disabled?: boolean }) {
