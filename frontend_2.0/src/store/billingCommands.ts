...
// ── finalize ───────────────────────────────────────────────────────────────────
export interface FinalizeInput extends DispatchMeta {
  invoiceId: Id
  customerId: Id
...
// ── edit a DRAFT invoice (before issuing) ────────────────────────────────────────
...
export interface EditDraftInput extends DispatchMeta {
  invoiceId: Id
  customerId?: Id
  issuerKind: IssuerKind
  issuerVendorId?: Id
...
const dispatchMeta = (i: DispatchMeta) => ({
  ewayBillNo: i.ewayBillNo?.trim() || undefined,
  vehicleNo: i.vehicleNo?.trim() || undefined,
  transporter: i.transporter?.trim() || undefined,
  dispatchedThrough: i.dispatchedThrough?.trim() || undefined,
  destination: i.destination?.trim() || undefined,
})

function validateEditDraft(s: RootState, input: EditDraftInput): { ok: true } | { ok: false; errors: string[] } {
  const inv = getById(s.billing.invoices, input.invoiceId)
  if (!inv) return { ok: false, errors: ['Invoice not found'] }
  // Only drafts are editable — an issued/void invoice is an immutable legal document.
  if (inv.lifecycle !== 'draft') return { ok: false, errors: [`Bill ${inv.billNo} is ${inv.lifecycle} — only a draft can be edited`] }
  if (!writableUnitIds(s).has(inv.unitId)) return { ok: false, errors: ["You don't have access to that unit"] }
  if (input.issuerKind === 'supplier') {
    const v = getById(s.masters.vendors, input.issuerVendorId)
    if (!v) return { ok: false, errors: ['Choose the issuing RM supplier'] }
    if (!v.gstin || !v.stateCode) return { ok: false, errors: ['The issuing supplier needs a GSTIN + state code'] }
  }
  return { ok: true }
}

function applyEditDraft(draft: RootState, input: EditDraftInput): ApplyOut<{ id: Id }> {
  const inv = getById(draft.billing.invoices, input.invoiceId) as Invoice
  const issuerId = input.issuerKind === 'supplier' && input.issuerVendorId ? input.issuerVendorId : inv.unitId
  patchEntity(draft.billing.invoices, inv.id, {
  ...
  return {
    result: { id: inv.id },
    cascade: [`Draft ${inv.billNo} updated`, 'still a draft — issue it when ready'],
    summary: `Edited draft invoice ${inv.billNo}`,
    refs: [{ type: 'invoice', id: inv.id }],
    unitId: inv.unitId,
  }
}

const editDraftCmd: Command<EditDraftInput, { id: Id }> = {
  name: 'editDraftInvoice', module: 'billing', action: 'edit', validate: validateEditDraft, apply: applyEditDraft,
}
export function runEditDraftInvoice(input: EditDraftInput): CommandResult<{ id: Id }> {
  return runCommand(editDraftCmd, input)
}
