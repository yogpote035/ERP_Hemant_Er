...
import { Badge, Button, Card, Drawer, EmptyState, SearchableDropdown, TablePager, type BadgeTone } from '@/components/ui'
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-end gap-2">
        <input
          className="input h-9 w-56 mr-auto"
          placeholder="Search…"
          aria-label="Search"
          value={paged.search}
      ...
      {form !== null ? (
        <UserFormDrawer key={form.user?.id ?? '__new__'} user={form.user} unitOpts={unitOpts} onClose={() => { setForm(null); bumpRefresh() }} />
      ) : null}
...
function RolesTab() {
  ...
  const paged = usePagedRows(roles, { pageSize: 25 })
