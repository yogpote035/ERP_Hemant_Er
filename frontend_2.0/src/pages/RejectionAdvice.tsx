...
import { selectRejectionRows, type RejectionRow } from '@/selectors/finance'
import { useCan } from '@/hooks/useCan'
import { toastCommandError, toastCommandSuccess } from '@/lib/commandToast'
import { Button, Card, ConfirmDialog, Drawer, EmptyState, SearchableDropdown, TablePager } from '@/components/ui'
...
export default function RejectionAdvice() {
  ...
  // Name lookups so an API-mode row (flat RejectionAdvice, IDs only) can be re-nested
  // into the RejectionRow shape the table renders. Hydrated even in API mode.
  const lookups = useStore(
    useShallow((s) => ({
      customers: s.masters.customers.byId,
      parts: s.masters.parts.byId,
      inwards: s.inventory.inwards.byId,
    }))
  )
  ...
  const paged = usePagedSource({
    localRows: rows,
    endpoint: '/rejection',
    searchText: (r) => `${r.advice.rejDcNo ?? ''} ${r.partNo ?? ''}`,
    pageSize: 25,
    refreshKey,
    mapApiRow: (raw): RejectionRow => {
      const a = raw as RejectionAdviceEntity
      return {
        advice: a,
        customerName: lookups.customers[a.customerId]?.name ?? '—',
        partNo: lookups.parts[a.partId]?.partNo ?? '—',
        challanNo: lookups.inwards[a.sourceInwardId]?.challanNo ?? '—',
        weightKg: a.totalWeightGrams / 1000,
      }
    },
  })
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-start gap-3">
        ...
        {can('rejection', 'create') ? (
          <Button leftIcon={<Plus size={15} />} onClick={() => setCreating(true)}>New rejection DC</Button>
        ) : null}
      </div>
      ...
      {creating ? <RejectionForm onClose={() => { setCreating(false); bumpRefresh() }} /> : null}
      {editing ? <RejectionForm existing={editing} onClose={() => { setEditing(null); bumpRefresh() }} /> : null}
      <ConfirmDialog
        open={deleting != null}
