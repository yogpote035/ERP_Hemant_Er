 ...
 */
export function groupMioRows(rows: unknown[][], map: MioColumnMap): GroupResult {
  ...
  rows.forEach((row, i) => {
    ...
    } else if (challanNo !== '' && recv == null) {
      // A challan no with no received qty on a non-opening row — treat as a
      // dispatch continuation but warn (the spine row should carry the qty).
      issues.push({ level: 'warn', row: sheetRow, message: `Challan ${challanNo} has no Received QTY; treated as a dispatch line` })
    }
