/**
 * Seed-on-first-load, Reset demo data, and JSON backup export/import
 * (plan §4 persistence). Backup is a versioned envelope validated on import
 * and run through the same migration chain as persist rehydrate.
 */
...
/** Seed only when the store has never been seeded (fresh localStorage). */
export function ensureSeeded(): void {
  if (!useStore.getState().system.seeded) {
    useStore.setState(seedState(), true)
  }
}

/** Wipe to a fresh deterministic seed (Settings → Reset demo data). */
