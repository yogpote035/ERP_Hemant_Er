...
import { runSaveProductionAttendance, runSaveShiftAttendance } from './attendanceCommands'
...
describe('production attendance', () => {
  it('derives makeQty + earned and snapshots the in-force rate', () => {
    const res = runSaveProductionAttendance({
      ...
      openingCounter: 1000, closingCounter: 1500, okQty: 480,
    })
  ...
  it('rejects OK qty greater than pieces made', () => {
    expect(() =>
      runSaveProductionAttendance({
        unitId: 'u1', date: '2025-05-01', employeeId: anEmployee().id, machineId: aMachine().id, partId: u1Part().id,
        openingCounter: 1000, closingCounter: 1500, okQty: 600,
      })
