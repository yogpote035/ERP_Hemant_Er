...
/** Derived per-(unit, part) stock for the units the session may read. */
export function selectStockRows(s: RootState): StockRow[] {
  ...
  const add = (unitId: Id, partId: Id) => {
    ...
    seen.add(key)
    const st = selectPartStock(s, unitId, partId)
