 ...
 */
import type { Expense, RejectionAdvice, ScrapBill } from '@/types/domain'
import { addP, subP, type Paise } from '@/lib/money'
import { todayISO } from '@/lib/date'
import { computeScrap, type ScrapMath } from '@/lib/scrapMath'
...
export function selectScrapRows(s: RootState): ScrapRow[] {
  const allowed = allowedUnitIds(s)
  return values(s.scrap.scrapBills)
    .filter((b) => allowed.has(b.unitId))
...
export function selectScrapTotals(s: RootState): ScrapTotals {
  const allowed = allowedUnitIds(s)
  let weightGrams = 0
  let revenue = ZERO
  let tcs = ZERO
  for (const b of values(s.scrap.scrapBills)) {
    if (!allowed.has(b.unitId)) continue
...
export function selectExpenseRows(s: RootState): ExpenseRow[] {
  const allowed = allowedUnitIds(s)
  return values(s.expenses.expenses)
    .filter((e) => allowed.has(e.unitId))
    .map((e) => ({
      expense: e,
...
export function selectVendorOutstanding(s: RootState): VendorOutstanding[] {
  const map = new Map<string, VendorOutstanding>()
  for (const row of selectExpenseRows(s)) {
...
export function selectRejectionRows(s: RootState): RejectionRow[] {
  const allowed = allowedUnitIds(s)
  return values(s.rejection.rejectionAdvices)
    .filter((r) => allowed.has(r.unitId))
    .map((r) => ({
      advice: r,
